...
class TkCalcCalibMergedChunk(Toplevel):
    ...
    def placeComponents(self):
        ...
        for chunk in interface.currentChunk.listChunks:
            ...
            self.checkAlign = Checkbutton(self, text="Use the alignement of "+chunk.nom, var=self.listAlign[-1])
...
class CanvasArborescence(Canvas):
    ...
    def drawHeader(self):
        ...
        self.create_text((x31+x32)/2, y1 + 3*self.sizey/4, text="and alignement", fill="white")
        self.create_text((x41+x42)/2, y1 + self.sizey/2, text="Cloud points", fill="white")
...
class TkInterfaceMicMac(Tk):
    ...
    def updateMenu(self, event):
        ...
        if self.selectedObject != 0 and self.selectedObject.done and (isinstance(self.selectedObject, CaliEtMEP) or isinstance(self.selectedObject, NuagePoints) or isinstance(self.selectedObject, Ortho)):
    ...
    def view(self):
        if isinstance(self.selectedObject, Ortho):
            ...
            print("\"C:\Program Files\CloudCompare\CloudCompare\" \"" + self.selectedObject.caliEtMEP.pointsLiaisons.chunk.dossier + "/"+self.selectedObject.nom+".ply\"")
            ...
            print("\"C:\Program Files\CloudCompare\CloudCompare\" \"" + self.selectedObject.pointsLiaisons.chunk.dossier + "/Align-"+self.selectedObject.nom+".ply\"")
    ...
    def load(self) :
        ...
        for line in ficLoad.readlines() :
            ...
            if line[0] == "Chunk" :
                ...
                ## Calcul d'une calibration et d'une mise en place pour chunk issu d'une fusion
                nbreCali = int(line[4])
                listCali = []
                listAlign = []
