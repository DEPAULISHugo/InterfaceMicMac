...
class TkBuildCloud(Toplevel):
    ...
    def callBackModeEqua(self):
        if self.equaRadio.get() == True :
            self.labelModeEqua.pack()
            self.modeEqua.pack()
        else :
            self.labelModeEqua.pack_forget()
            self.modeEqua.pack_forget()
    ...
    def placeComponents(self):
        ...
        self.checkEqua = Checkbutton(self, text="Radiometric equalization", var=self.equaRadio, command=self.callBackModeEqua)
        self.checkEqua.pack()
        self.labelModeEqua = Label(self,text="Equalization mode:", anchor=W, width=30)
        self.modeEqua = ttk.Combobox(self,values=["Midway Equalization", "Equalization by geometry"],width=27)
        frame = Frame(self)
        Button(frame,text="Ok",command=self.validate,width=12).pack(side=LEFT,padx=5,pady=7)
        Button(frame,text="Cancel",command=self.destroy,width=12).pack(side=RIGHT,padx=5,pady=7)
        frame.pack(side=BOTTOM)
    ...
    def validate(self):
        ...
        modeEqua = self.modeEqua.get()
        newNuagePoints = NuagePoints(name, mode, offset, equaRadio, modeEqua, interface.currentCaliEtMEP)
...
class TkBuildOrtho(Toplevel):
    ...
    def placeComponents(self):
        ...
        self.equaRadio = BooleanVar()
        self.equaRadio.set(False)
        self.checkEqua = Checkbutton(self, text="Radiometric equalization", var=self.equaRadio)
        self.checkEqua.pack()
...
class TkMergePointsCloud(Toplevel):
    def __init__(self) :
        super().__init__(interface)
        self.title("Merge by points clouds")
        self.placeComponents()
...
class TkInterfaceMicMac(Tk):
    ...
    def generateMenu(self):
        ...
        self.workflowmenu.add_command(label="Merge by points cloud", command=TkMergePointsCloud)
    ...
    def load(self) :
        ...
        for line in ficLoad.readlines() :
            ...
            if line[0] == "Chunk" :
                ...
                if line[4] == "True" :
                    nuagePointsLoaded = NuagePoints(line[1], line[2], line[3], True, line[5], self.currentCaliEtMEP)
                else :
                    nuagePointsLoaded = NuagePoints(line[1], line[2], line[3], False, None, self.currentCaliEtMEP)
                if line[7] == "done" :
