...
class PointsLiaisonsMulscale(PointsLiaisons) :
    ...
    def run(self, interface) :
        ...
        interface.canvas.draw()
        os.chdir(self.chunk.dossier)
        os.system("mm3d Tapioca " + "Muscale" + " \".*" + self.chunk.extension + "\" " + self.lowResolution + " " + self.resolution + " PostFix=" + self.nom)
        self.getNbrePtsLiaisons()
        self.ongoing =False
        self.done = True
        interface.canvas.draw()
...
class NuagePoints() :
    ...
    def getNbrePoints(self) :
        ...
        fic = open(self.nom+".ply","r")
        for line in fic.readlines() :
            if "element vertex" in line :
                self.nbrePoints = line.split(" ")[2]
                return
...
class Mesh() :
    ...
    def run(self, interface) :
        ...
        if self.textured :
            os.system("mm3d Tequila .*" + self.nuagePoints.caliEtMEP.pointsLiaisons.chunk.extension + " " + self.nuagePoints.caliEtMEP.nom + " " + self.nom + ".ply " + "Out=" + self.nom + "-textured.ply")
