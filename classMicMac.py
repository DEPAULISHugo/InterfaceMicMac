...
import numpy as np
import histogramEqualisation as he
...
class NuagePoints() :
    def __init__(self, nom, rendu, offset, egalRadio, modeEgal, caliEtMEP) :
        self.nom = nom
        self.rendu = rendu
        self.offset = offset
        self.listOrthos = []
        ...
        self.nbrePoints = "???"
        self.egalRadio = egalRadio
        self.modeEgal = modeEgal
        self.done = False
        self.ongoing = False
    ...
    def run(self, interface) :
        ...
        #Egalisation radiométrique
        if self.egalRadio == True :
            os.system("md Equa-" + self.nom)
            if self.modeEgal == "Midway Equalization" :
                listPhotos = []
                for fic in os.listdir(self.caliEtMEP.pointsLiaisons.chunk.dossier) :
                    if "." + self.caliEtMEP.pointsLiaisons.chunk.extension in fic :
                        listPhotos.append(fic)
                equa = he.histogramEqualizer(listPhotos, self.caliEtMEP.pointsLiaisons.chunk.dossier + "/Equa-" + self.nom)
                equa.equalizeMidway()
                os.system("rd /s /q Tmp-MM-Dir")
                os.system("md savePhotos")
                os.system("copy /y *.JPG savePhotos")
                os.system("copy /y Equa-" + self.nom + "\*.JPG")
                

        patternImages = ".*" + self.caliEtMEP.pointsLiaisons.chunk.extension
        
        if self.rendu == "Medium" :
            os.system("mm3d C3DC QuickMac " + patternImages + " " + self.caliEtMEP.nom + " OffsetPly=" + self.offset + " Out=" + self.nom + ".ply\n")
            os.system("ren PIMs-QuickMac PIMs-QuickMac-" + self.nom)
        elif self.rendu == "Good" :
            os.system("mm3d C3DC MicMac " + patternImages + " " + self.caliEtMEP.nom + " OffsetPly=" + self.offset + " Out=" + self.nom + ".ply\n")
            os.system("ren PIMs-MicMac PIMs-MicMac-" + self.nom)
        elif self.rendu == "Excellent" :
            os.system("mm3d C3DC BigMac " + patternImages + " " + self.caliEtMEP.nom + " OffsetPly=" + self.offset + " Out=" + self.nom + ".ply\n")
            os.system("ren PIMs-BigMac PIMs-BigMac-" + self.nom)
        elif self.rendu == "Highest" :
            os.system("mm3d C3DC Statue " + patternImages + " " + self.caliEtMEP.nom + " OffsetPly=" + self.offset + " Out=" + self.nom + ".ply\n")
            os.system("ren PIMs-Statue PIMs-Statue-" + self.nom)
            for file in os.listdir(self.caliEtMEP.pointsLiaisons.chunk.dossier) :
                if "MTD-Image" in file :
                    os.system("rd /s /q " + file)
                elif "Masq1_Glob.tif" in file :
                    os.system("del /q " + file)
        os.system("ren Homol Homol" + self.caliEtMEP.pointsLiaisons.nom)
        
        if self.egalRadio == True :
            os.system("rd /s /q Tmp-MM-Dir")
            os.system("copy /y savePhotos\*.JPG")
            os.system("rd /s /q savePhotos")
    ...
    def save(self) :
        saveText = "NuagePoints\t" + self.nom + "\t" + self.rendu + "\t" + self.offset + "\t"
        if self.egalRadio :
            saveText += "True" + "\t" + self.modeEgal + "\t" + self.caliEtMEP.nom
        else :
            saveText += "False" + "\t" + "None" + "\t" + self.caliEtMEP.nom
        if self.done :
            saveText += "\tdone\n"
        else :
            saveText += "\tplanned\n"
...
class NuagePointsMerge(NuagePoints) :
    def __init__(self, nom, rendu, offset, targetPC, sourcePC, matrix, maxIteration, sampling, caliEtMEP) :
        NuagePoints.__init__(self, nom, rendu, offset, False, None, caliEtMEP)
...
class Ortho() :
    ...
    def run(self, interface) :
        ...
        if self.nuagePoints.egalRadio == True :
            #Preparation du bon set de photos
            os.system("rd /s /q Tmp-MM-Dir")
            os.system("md savePhotos")
            os.system("copy /y *.JPG savePhotos")
            os.system("copy /y Equa-" + self.nom + "\*.JPG")
        ...
        if self.nuagePoints.egalRadio == True :
            #Remise en place du bon set de photos
            os.system("rd /s /q Tmp-MM-Dir")
            os.system("copy /y savePhotos\*.JPG")
            os.system("rd /s /q savePhotos")
...
class Mesh() :
    ...
    def run(self, interface) :
        ...
        #Création de la nouvelle orientation prenant en compte l'offset du nuage
        self.correctOffset()
        ...
        patternImages = ".*" + self.nuagePoints.caliEtMEP.pointsLiaisons.chunk.extension
            
        #os.system("mm3d TiPunch " + self.nuagePoints.nom + ".ply Filter=true Out=" + self.nom + ".ply Pattern=" + patternImages + " Depth=" + str(self.depth))
        os.system("mm3d TiPunch " + self.nuagePoints.nom + ".ply Filter=false Out=" + self.nom + ".ply Pattern=" + patternImages + " Depth=" + str(self.depth))
        if self.textured :
            os.system("mm3d Tequila " + patternImages + " " + self.nuagePoints.caliEtMEP.nom + "-Texture " + self.nom + ".ply " + " Mode=Basic Out=" + self.nom + "-textured.ply")
            #os.system("mm3d Tequila " + patternImages + " " + self.nuagePoints.caliEtMEP.nom + "-Texture " + self.nom + ".ply " + " Mode=Basic Out=" + self.nom + "-textured.ply")
            #os.system("mm3d Tequila " + patternImages + " " + self.nuagePoints.caliEtMEP.nom + "-Texture " + self.nom + ".ply " + " Out=" + self.nom + "-textured.ply")
    ...
    def correctOffset(self) :
        os.system("md Ori-" + self.nuagePoints.caliEtMEP.nom + "-Texture")
        os.system("copy /y Ori-" + self.nuagePoints.caliEtMEP.nom + "\* " + "Ori-" + self.nuagePoints.caliEtMEP.nom + "-Texture")
        
        offsetNuage = self.nuagePoints.offset[1:-1].split(",")
        offsetNuage = np.array([float(offsetNuage[0]), float(offsetNuage[1]), float(offsetNuage[2])])
        for file in os.listdir("Ori-" + self.nuagePoints.caliEtMEP.nom + "-Texture") :
            if "Orientation-" in file :
                newFile = []
                fileOri = open("Ori-" + self.nuagePoints.caliEtMEP.nom + "-Texture/" + file, "r")
                for line in fileOri.readlines() :
                    if "<Centre>" in line :
                        debut = line.index("<Centre>") + 8
                        fin   = line.index("</Centre>")
                        coordinates = line[debut:fin].split(" ")
                        vector = np.array([float(coordinates[0]), float(coordinates[1]), float(coordinates[2])]) - offsetNuage
                        newLine = line[:debut] + str(vector[0]) + " " + str(vector[1]) + " " + str(vector[2]) + line[fin:]
                        newFile.append(newLine)
                    else :
                        newFile.append(line)
                fileOri.close()
                fileOri = open("Ori-" + self.nuagePoints.caliEtMEP.nom + "-Texture/" + file, "w")
                for line in newFile :
                    fileOri.write(line)
                fileOri.close()
